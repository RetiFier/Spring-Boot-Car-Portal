...
public class PostsController {
  ...
  public ModelAndView create(@Valid Post post, Bid bid, BindingResult bindingResult) {
    ModelAndView modelAndView = new ModelAndView();
    modelAndView.setViewName("posts/create");
    // Perform validation
    ...
    if (post.getModel().isEmpty()) {
      bindingResult.rejectValue("model", "error.post", "Model cannot be empty");
    }
    if (post.getBrand().isEmpty()) {
      bindingResult.rejectValue("brand", "error.post", "Brand cannot be empty");
    }
    if (post.getColor().isEmpty()) {
      bindingResult.rejectValue("color", "error.post", "Color cannot be empty");
    }

    if (post.getEnginePower().isEmpty()) {
      bindingResult.rejectValue("enginePower", "error.post", "Engine Power cannot be empty");
    }
    if (post.getEnginePower().isEmpty()) {
      bindingResult.rejectValue("enginePower", "error.post", "Engine Power cannot be empty");
    }

    if (post.getCarLicensePlate().isEmpty()) {
      bindingResult.rejectValue("carLicensePlate", "error.post", "Car License Plate cannot be empty");
    }
    if (post.getStartPrice().isEmpty()) {
      bindingResult.rejectValue("startPrice", "error.post", "Start Price cannot be empty");
    }
    // Get author
    Authentication auth = SecurityContextHolder.getContext().getAuthentication();
    User user = this.userService.findByEmail(auth.getName());
    User buser = this.userService.findByEmail(auth.getName());
    // Get Bidding

    bid.setBidUser(buser);
    bid.setBidPrice(post.getStartPrice());
    bid.setDate(post.getDate());
    ...
    // bid.setItem(this.postService.findById(post.getId()));
    bid.setItem(post);
    this.bidService.create(bid);

    Post postsedit = this.postService.findById(post.getId());
    postsedit.setBid(bid);
    this.postService.edit(postsedit);
